<?= head_(); ?>
<?= header_(); ?>
<div id="s26-withholdings-view" class="container-fluid s26-container" tabindex="0">
  <?php
  ...
  <?php } else { ?>
    <div class="row align-items">
      <s26-sidebar title="Retenciones" icon="file-alt" @update="allRows" @reset="onReset" v-model="activeSidebar" :url_export="url_export">
          ...
          <div class="container">
            <div class="row">
              <div class="col-12">
                ...
                <s26-tarjet-info title="Ret. Iva" variant="danger" icon="money-bill-wave">
                  <s26-icon icon="dollar-sign" class="text-danger"></s26-icon>
                  {{ $s26.currency(s26_data.info.total_ret_iva) }}
                </s26-tarjet-info>
                <s26-tarjet-info title="Ret. Imp. Rent" variant="purple" icon="money-bill-wave">
                  <s26-icon icon="dollar-sign" class="s26-text-purple"></s26-icon>
                  {{ $s26.currency(s26_data.info.total_ret_imp_rent) }}
                </s26-tarjet-info>
              </div>
            </div>
          </div>
      ...
      <s26-table :fields="fields" :rows="s26_data.info.count" @get="allRows" :sidebar="activeSidebar" v-model="filter.perPage" action>
        <template v-slot:body>
          <tr v-for="item in s26_data.items" :key="item.id">
            ...
            <td class="length-description">{{ item.n_document }}</td>
            <td class="length-description">
              {{ item.business_name }}
            </td>
            <td class="length-action">{{ $s26.currency(item.ret_iva) }}</td>
            <td class="length-action">{{ $s26.currency(item.ret_imp_rent) }}</td>
            <td class="length-action">
              {{ $s26.currency(item.total) }}
            </td>
      ...
      ?>
        ...
        <transition name="slide-fade">
          <s26-read-withholding v-model="action" :id="idRow" v-if="action == 'watch'">
          </s26-read-withholding>
        </transition>
      <?php
      }
      ...
      ?>
        <!-- Modal Nuevo-->
        <transition name="slide-fade">
          <s26-form-withholding v-model="action" :id="idRow" v-if="action == 'update'" @update="allRows"></s26-form-withholding>
        </transition>
      <?php
      }
      ?>
      ...
      ?>
        <!-- Modal Eliminar -->
        <transition name="slide-fade">
          <s26-delete v-model="action" @update="allRows" v-if="action == 'delete'" :post_delete="'withholdings/delWithholding/' + idRow"></s26-delete>
        </transition>
      <?php
      }
      ?>
