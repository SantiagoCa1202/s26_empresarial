<?= head_(); ?>
<?= header_(); ?>
<div id="s26-creditNotes-view" class="container-fluid s26-container" tabindex="0">
  <?php
  ...
  <?php } else { ?>
    <div class="row align-items">
      <s26-sidebar title="Notas de Crédito" icon="file-alt" @update="allRows" @reset="onReset" v-model="activeSidebar" :url_export="url_export">
        ...
        <template v-slot:body>
          <tr v-for="item in s26_data.items" :key="item.id">
            ...
            <td class="length-action text-center">
              <a :href="item.file.href" v-if="item.file !== ''" target="_blank" :download="'nota_de_credito_' + item.n_document">
                <s26-icon icon="file-pdf" class="text-danger"></s26-icon>
              </a>
              <s26-icon icon="file-pdf" class="text-secondary" v-else></s26-icon>
            </td>
            <td class="length-action text-center">
              {{ item.buy.establishment.n_establishment.padStart(3,'0') }}
            </td>
      ...
      ?>
        ...
        <transition name="slide-fade">
          <s26-read-credit-note v-model="action" :id="idRow" v-if="action == 'watch'">
          </s26-read-credit-note>
        </transition>
      <?php
      }
      ...
      ?>
        <!-- Modal Nuevo-->
        <transition name="slide-fade">
          <s26-form-credit-note v-model="action" :id="idRow" v-if="action == 'update'" @update="allRows"></s26-form-credit-note>
        </transition>
      <?php
      }
      ?>
      ...
      ?>
        <!-- Modal Eliminar -->
        <transition name="slide-fade">
          <s26-delete v-model="action" @update="allRows" v-if="action == 'delete'" :post_delete="'creditNotes/delCreditNote/' + idRow"></s26-delete>
        </transition>
      <?php
      }
      ?>
