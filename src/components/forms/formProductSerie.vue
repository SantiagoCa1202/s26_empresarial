<template>
  <s26-modal @hideModal="$emit('input', null)">
    <template v-slot:header>
      <h1 class="modal-title h4">Editar Serie</h1>
    </template>
      ...
      <div class="row">
        <div class="col-12 mb-3">
          <h2 class="text-center h6">
            {{ form.name }} - {{ form.model }} - {{ form.trademark }}
          </h2>
        </div>
        <div class="col-12">
          <s26-select-buys
...
export default {
  ...
  methods: {
    infoData(id) {
      this.axios
        .get("/productsSeries/getSerie/" + id)
    ...
    onSubmit() {
      ...
      if (this.form.document_id >= -1) {
        this.$alertify.confirm(
          `Desea Asignar Documento a Serie?`,
          () => {
            let formData = $s26.json_to_formData(this.form);
            $s26.show_loader_points();
            this.axios
              .post("/productsSeries/setSerie", formData)
              .then((res) => {
                if (res.data.type == 1) {
                  this.onReset();
