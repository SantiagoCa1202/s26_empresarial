...
export default {
  ...
  computed: {
    select: function () {
      ...
      if (this.value != 0) {
        ...
        this.axios
          .get("/customers/getCustomer/" + val)
          .then(
            (res) =>
              (this.selected = `${res.data.document} - ${res.data.full_name}`)
          )
  ...
  methods: {
    ...
    getRow() {
      let val = this.all_info ? this.value.id : this.value;
      $s26.create_cookie("id", val, "customers");
      window.open(BASE_URL + "/customers", "_blank");
    },
  },
};
</script>
