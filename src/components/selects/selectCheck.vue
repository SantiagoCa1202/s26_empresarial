  ...
  <div :id="'s26-custom-select-' + id" class="s26-custom-select mb-3">
    ...
    <div class="s26-select-container">
      ...
      <div class="s26-select-container-options">
        ...
        >
          {{ option.n_check }} - $ {{ $s26.currency(option.amount) }} -
          {{ option.bank_account.bank_entity.bank_entity }}
        </div>
      </div>
...
export default {
  ...
  computed: {
    select: function () {
      ...
      if (this.value != 0) {
        this.axios
          .get("/checkBooks/getCheckBook/" + this.value)
          .then(
            (res) =>
              (this.selected =
                res.data.n_check +
                " - $ " +
                $s26.currency(res.data.amount) +
                " - " +
                res.data.bank_account.bank_entity.bank_entity)
          )
  ...
  methods: {
    ...
    getRow() {
      $s26.create_cookie("id", this.value, "checkBooks");
      window.open(BASE_URL + "/checkBooks", "_blank");
    },
  },
};
</script>
