  ...
  <div :id="'s26-custom-select-' + id" class="s26-custom-select mb-3">
    ...
    <div class="s26-select-container">
      ...
      <div class="s26-select-container-options">
        ...
        >
          {{ option.name }} / {{ option.establishment }}
        </div>
      </div>
...
export default {
  ...
  computed: {
    select: function () {
      ...
      if (this.value != 0) {
        this.axios
          .get("/boxes/getBox/" + this.value)
          .then(
            (res) =>
              (this.selected = `${res.data.name} / ${res.data.establishment}`)
          )
  ...
  methods: {
    ...
    getRow() {
      $s26.create_cookie("id", this.value, "boxes");
      window.open(BASE_URL + "/boxes", "_blank");
    },
  },
};
</script>
