...
export default {
  ...
  computed: {
    select: function () {
      ...
      if (this.value != 0) {
        this.axios
          .get("/bankAccounts/getBankAccount/" + this.value)
          .then((res) => (this.selected = res.data.bank_entity.bank_entity))
  ...
  methods: {
    ...
    getRow() {
      $s26.create_cookie("id", this.value, "bankAccounts");
      window.open(BASE_URL + "/bankAccounts", "_blank");
    },
  },
};
</script>
